...
class DQNBaseAgent(ABC):
    ...
    def train(self):
        ...
        while self.total_time_step <= self.training_steps:
            observation, info = self.env.reset() # observation: 即當前的遊戲畫面, shape: (210, 160, 3)
            episode_reward = 0 # total reward
            episode_len = 0 # length of path
            episode_idx += 1 # index of states in the current path
            while True:
                if self.total_time_step < self.warmup_steps:
                    ...
                    self.epsilon_decay() # decrease the probability of random sampling of actions
                # terminated=True if environment terminates 
                ...
                if self.total_time_step >= self.warmup_steps: # already collected enough steps
                    self.update()
